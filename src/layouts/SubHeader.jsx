...
import { Save, User, Shield, CreditCard, FileText, HelpCircle, LogOut, Cog, Users, FolderKanban, Wallet, Archive, Globe, LayoutTemplate, Plus, Trash2, FolderCog, Lock, LayoutDashboard, ListChecks, Table, AreaChart, Calendar, Layers, PieChart, BookOpen, Receipt, Hash, Folders, Share2, Gift } from 'lucide-react';
...
const SubHeader = () => {
  ...
  const projectNavItem = { label: 'Mes projets', path: '/app/projets', icon: Folders };
  ...
  const visibleNavItems = useMemo(() => {
    ...
    const accessibleLabelsMap = {
        ...
        accounts: ['Budget'],
        trezo: ['Budget', 'Trezo'],
        flux: ['Budget', 'Trezo', 'Flux'],
        echeancier: ['Budget', 'Trezo', 'Flux', 'Echeancier'],
        analyse: ['Budget', 'Trezo', 'Flux', 'Echeancier', 'Analyse'],
    };
        ...
        <div className="w-full px-6">
          <div className="py-2 grid grid-cols-[auto_1fr_auto] w-full items-center gap-6">
            <div className="flex items-center gap-4">
              <NavLink to="/app/projets" className="flex items-center gap-2 text-gray-700 hover:text-blue-600">
            ...
            <div className="flex items-center justify-center gap-6">
              {location.pathname !== '/app/projets' && (
                <nav className="flex items-center gap-1">
                  <NavLink to={projectNavItem.path} className={navLinkClasses} title={projectNavItem.label}>
                      <projectNavItem.icon size={20} />
                  </NavLink>
                  {visibleNavItems.length > 0 && <div className="h-6 w-px bg-gray-300 mx-2" />}
                  {visibleNavItems.map(item => (
                      item && <NavLink key={item.label} to={item.path} className={navLinkClasses} title={item.label}>
                          <item.icon size={20} />
                      </NavLink>
                  ))}
                  <div className="h-6 w-px bg-gray-300 mx-2" />
                  <NavLink to="/app/parrainage" className={navLinkClasses} title="Programme Ambassadeur">
                      <Gift size={20} />
                  </NavLink>
                </nav>
              )}
