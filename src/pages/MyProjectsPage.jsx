...
import { Plus, Layers, Folder, Briefcase, Home, PartyPopper } from 'lucide-react';
import ProjectCard from '../components/ProjectCard';
import ConsolidatedViewCard from '../components/ConsolidatedViewCard';
import EmptyState from '../components/EmptyState';
import ProjectModal from '../components/ProjectModal';
import { saveProject, deleteProject } from '../context/actions';
...
const MyProjectsPage = () => {
    ...
    const myConsolidatedViews = useMemo(() => consolidatedViews.filter(view => {
        return view.project_ids.every(pid => myTreasuryProjects.some(mp => mp.id === pid));
    }), [consolidatedViews, myTreasuryProjects]);

    const totalProjects = allNonArchivedProjects.length;

    const handleNewTreasuryProject = () => {
        uiDispatch({ type: 'START_ONBOARDING', payload: { projectClass: 'treasury' } });
    };

    const handleNewEventProject = () => {
        uiDispatch({ type: 'START_ONBOARDING', payload: { projectClass: 'event' } });
    };
    ...
    return (
        <div className="p-6 max-w-full space-y-12">
            ...
            {/* My Treasury Projects */}
            <section>
                <div className="flex justify-between items-center mb-4">
                    <h2 className="text-xl font-semibold text-gray-700">Mes projets de trésorerie</h2>
                    <button
                        onClick={handleNewTreasuryProject}
                ...
                <section>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                        {myConsolidatedViews.map(view => (
                            <ConsolidatedViewCard key={view.id} view={view} />
                        ))}
                    </div>
                </section>
            )}
            ...
            <section>
                <div className="flex justify-between items-center mb-4">
                    <h2 className="text-xl font-semibold text-gray-700">Mes événements</h2>
                    <button
                        onClick={handleNewEventProject}
                        className="flex items-center gap-2 px-4 py-2 text-sm font-semibold text-white bg-green-600 rounded-lg hover:bg-green-700 transition-colors shadow"
                    >
                        <Plus size={16} />
                        Nouveau événement
                    </button>
                 ...
                 {myEventProjects.length > 0 ? (
                    ...
                    <div className="text-center py-8 border-2 border-dashed rounded-lg text-gray-500">
                        ...
                        <p className="text-sm">Créez des projets ponctuels comme un mariage, un voyage ou une construction.</p>
                    </div>
                )}
            </section>

            {/* Shared Event Projects */}
