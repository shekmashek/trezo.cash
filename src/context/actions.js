  ...
  try {
    ...
    if (projectData.id) {
      ...
      dataDispatch({
        ...
        payload: { projectId: data.id, newSettings: { name: data.name, description: data.description } }
      });
      uiDispatch({ type: 'ADD_TOAST', payload: { message: 'Projet mis à jour.', type: 'success' } });
    } else {
      // Create new project
      const { name, description, startDate, endDate, isEndDateIndefinite, currencyCode, currencySymbol, projectType } = projectData;
...
export const saveEntry = async ({ dataDispatch, uiDispatch, dataState }, { entryData, editingEntry, user, tiers, cashAccounts, exchangeRates }) => {
    try {
        const activeProjectId = entryData.projectId;
        const { collaborators, allEntries } = dataState;

        // --- Budget Limit Check ---
        const collaboratorRecord = collaborators.find(c => c.userId === user.id && c.projectIds?.includes(activeProjectId));
        ...
        if (collaboratorRecord && collaboratorRecord.role === 'editor' && collaboratorRecord.budgetLimit != null) {
            ...
            const collaboratorEntries = (allEntries[activeProjectId] || []).filter(e => e.userId === user.id);
...
export const inviteCollaborator = async ({ dataDispatch, uiDispatch }, { email, role, permissionScope, projectIds, budgetLimit }) => {
    try {
        const { data, error } = await supabase.rpc('invite_collaborator', {
            ...
            p_budget_limit: budgetLimit ? parseFloat(budgetLimit) : null,
        });

        if (error) {
            // Specific check for an already invited user. This is not a system error, but user info.
            ...
            // For other, unexpected errors, throw them to be caught by the catch block.
            throw new Error(error.message.replace('error: ', ''));
        }

        if (data) {
            const { new_collaborator: newCollaborator, new_profile: newProfile } = data;
            ...
            dataDispatch({ 
                ...
                payload: { newCollaborator, newProfile } 
            });
        }
