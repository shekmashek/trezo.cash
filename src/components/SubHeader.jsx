...
import { User, Shield, CreditCard, FileText, HelpCircle, LogOut, Cog, Users, FolderKanban, Archive, Globe, LayoutTemplate, Trash2, FolderCog, Lock, Gift, Banknote, Hash, UserPlus, Check } from 'lucide-react';
...
const SubHeader = () => {
    ...
    const showCollaboratorsSection = isSingleProject || isCustomConsolidated;
                    ...
                    <div className="flex items-center justify-end gap-4">
                        ...
                        <div className="flex items-center gap-2">
                             <div className="relative" ref={langMenuRef}>
                                ...
                                >
                                    <FlagIcon code={currentLang} className="w-6 h-6 rounded-full object-cover" />
                                </button>
                                <AnimatePresence>
                                    {isLangMenuOpen && (
                                        <motion.div
                                        ...
                                        >
                                            <ul className="p-1">
                                                <li>
                                                    <button
                                                        onClick={() => { setCurrentLang('FR'); setIsLangMenuOpen(false); }}
                                                        className="w-full text-left flex items-center gap-3 px-3 py-2 text-sm rounded-md text-gray-700 hover:bg-gray-100"
                                                    >
                                                        <FlagIcon code="FR" className="w-5 h-5 rounded-sm" />
                                                        <span>Français</span>
                                                        {currentLang === 'FR' && <Check className="w-4 h-4 text-blue-600 ml-auto" />}
                                                    </button>
                                                </li>
                                                <li>
                                                    <button
                                                        onClick={() => { setCurrentLang('GB'); setIsLangMenuOpen(false); }}
                                                        className="w-full text-left flex items-center gap-3 px-3 py-2 text-sm rounded-md text-gray-700 hover:bg-gray-100"
                                                    >
                                                        <FlagIcon code="GB" className="w-5 h-5 rounded-sm" />
                                                        <span>English</span>
                                                        {currentLang === 'GB' && <Check className="w-4 h-4 text-blue-600 ml-auto" />}
                                                    </button>
                                                </li>
                                            </ul>
                                        </motion.div>
