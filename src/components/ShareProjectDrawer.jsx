...
import { X, Send, Trash2, Users, DollarSign } from 'lucide-react';
...
const ShareProjectDrawer = ({ isOpen, onClose }) => {
    ...
    const { session, projects, collaborators, allProfiles, consolidatedViews } = dataState;
    ...
    const isCustomConsolidated = useMemo(() => activeProjectId?.startsWith('consolidated_view_'), [activeProjectId]);
    ...
    }, [activeProjectId, projects, consolidatedViews, isCustomConsolidated]);
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            setBudgetLimit(''); // Reset on open
        }
    }, [isCustomConsolidated, isOpen]);
    ...
    const projectCollaborators = useMemo(() => {
        if (!activeProjectId || activeProjectId.startsWith('consolidated')) return { active: [], pending: [] };
        ...
        collaborators.forEach(c => {
            if (c.projectIds?.includes(activeProjectId)) {
                if (c.status === 'accepted') {
                    const profile = allProfiles.find(p => p.id === c.userId);
                    if (profile) {
    ...
    }, [activeProjectId, collaborators, allProfiles, projects]);
    ...
    const handleInvite = (e) => {
        ...
        let projectIdsToShare = [];
        if (isCustomConsolidated && activeProjectOrView) {
            projectIdsToShare = activeProjectOrView.project_ids;
        } else if (activeProjectId) {
            projectIdsToShare = [activeProjectId];
        }

        if (projectIdsToShare.length === 0) {
            uiDispatch({ type: 'ADD_TOAST', payload: { message: "Aucun projet à partager.", type: 'error' } });
            return;
        }

        inviteCollaborator({ dataDispatch, uiDispatch }, {
            email: inviteEmail,
            role: isCustomConsolidated ? 'viewer' : inviteRole,
            permissionScope: inviteScope,
            projectIds: projectIdsToShare,
            ownerId: session.user.id,
            budgetLimit: inviteRole === 'editor' ? budgetLimit : null,
        });
        setInviteEmail('');
    };
            ...
            <div className="fixed top-0 right-0 bottom-0 w-full max-w-lg bg-gray-50 shadow-xl z-50 transform transition-transform duration-300 ease-in-out">
                <div className="flex flex-col h-full">
                    <div className="flex items-start justify-between p-4 border-b bg-white">
                        <div>
                            <h2 className="text-lg font-semibold text-gray-800">Partager "{activeProjectOrView?.name}"</h2>
                            ...
                            <form onSubmit={handleInvite} className="space-y-4">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <select value={inviteRole} onChange={(e) => setInviteRole(e.target.value)} disabled={isCustomConsolidated} className="w-full px-3 py-2 border rounded-lg text-sm bg-white disabled:bg-gray-100 disabled:cursor-not-allowed">
                                            <option value="viewer">Lecteur</option>
                                            <option value="editor">Éditeur</option>
                                        </select>
                                        {isCustomConsolidated && <p className="text-xs text-gray-500 mt-1">Lecture seule pour les vues consolidées.</p>}
                                    </div>
                                    <div>
                                ...
                                <AnimatePresence>
                                    {inviteRole === 'editor' && !isCustomConsolidated && (
                                        <motion.div
                                        ...
                                        >
                                            <div>
                                                <label className="block text-xs font-medium text-gray-600 mb-1">Plafond budgétaire (optionnel)</label>
                                                ...
                                                <p className="text-xs text-gray-500 mt-1">L'éditeur ne pourra pas créer/modifier des entrées dépassant ce total.</p>
                                            </div>
                                        </motion.div>
                                    )}
                                </AnimatePresence>
