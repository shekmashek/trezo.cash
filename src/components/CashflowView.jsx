...
const CashflowView = ({ isFocusMode = false }) => {
  ...
  const { settings, allEntries, allCashAccounts } = dataState;
  const { timeUnit, horizonLength, periodOffset, activeQuickSelect, activeProjectId, onboardingCompletedProjects } = uiState;
  ...
  const projectHasEntries = useMemo(() => {
      if (!activeProjectId || isConsolidated) return true;
      const entries = allEntries[activeProjectId];
      return entries && entries.length > 0;
  }, [activeProjectId, allEntries, isConsolidated]);

  const projectHasConfiguredAccounts = useMemo(() => {
      if (isConsolidated || !activeProjectId) return true;
      const accounts = allCashAccounts[activeProjectId];
      return accounts && accounts.length > 0;
  }, [activeProjectId, allCashAccounts, isConsolidated]);
  ...
  return (
    <div className={isFocusMode ? "h-full flex flex-col" : "p-6 max-w-full flex flex-col h-full"}>
      {showOnboardingButton && !isFocusMode && (
          <div className="text-center mb-6">
              ...
              >
                  Valider mon flux et voir l'analyse de mes entrée et sorties
              </button>
          </div>
      )}
      {!isFocusMode && (
