...
const ReferralDashboard = () => {
    ...
    useEffect(() => {
        const fetchReferralData = async () => {
            ...
            try {
                const { data: profile, error: profileError } = await supabase.from('profiles').select('referral_code').eq('id', session.user.id).single();
                if (profileError) throw profileError;

                const { data: referrals, error: referralsError } = await supabase.from('referrals').select('referred_id').eq('referrer_id', session.user.id);
                if (referralsError) throw referralsError;

                const refereeIds = referrals.map(r => r.referred_id);
                let referees = [];
                if (refereeIds.length > 0) {
                    const { data: refereeProfiles, error: refereeProfilesError } = await supabase.from('profiles').select('id, full_name, email, subscription_status, created_at').in('id', refereeIds);
                    if (refereeProfilesError) throw refereeProfilesError;
                ...
                const validatedReferrals = referees.filter(r => r.subscription_status === 'active' || r.subscription_status === 'lifetime');
                const validatedCount = validatedReferrals.length;
                const currentLevel = levels.slice().reverse().find(l => validatedCount >= l.min_referrals) || levels[0];
                const nextLevel = levels.find(l => validatedCount < l.min_referrals);

                setReferralData({
                    referralCode: profile.referral_code,
                    invitedCount: referrals.length,
                    validatedCount: validatedCount,
                    conversionRate: referrals.length > 0 ? (validatedCount / referrals.length) * 100 : 0,
                    generatedRevenue: validatedCount * 12 * 12, // 12€/month * 12 months
                    referees: referees.sort((a, b) => new Date(b.created_at) - new Date(a.created_at)),
                    levels: levels,
